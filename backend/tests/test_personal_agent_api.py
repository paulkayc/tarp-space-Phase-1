...
def test_personal_agent_send_message_and_memory_question(client, user_uuid):
    ...
    send_response = client.post(
        ...
        json={
            "content": (
                "Call me Alex. I'm in Houston. I prefer detailed responses and modern style. "
                "Please avoid stairs."
            )
        },
    )
    ...
    assert turn["persona"]["preferred_name"] == "Alex"
    assert turn["persona"]["city_or_area"] == "Houston"
    assert turn["persona"]["communication_style"] == "detailed"
    assert "modern" in turn["persona"]["style_preferences"]
    assert "stairs" in turn["persona"]["personal_constraints"][0]
    ...
    assert "alex" in memory_turn["agent_message"]["content"].lower()


def test_personal_agent_deflects_mandate_requests(client, user_uuid):
    create_response = client.post(
        "/api/v1/personal-agent/sessions",
        headers={"X-Dev-User-Id": user_uuid},
    )
    ...
    send_response = client.post(
        f"/api/v1/personal-agent/sessions/{conversation_id}/messages",
        headers={"X-Dev-User-Id": user_uuid},
        json={"content": "Find me a condo listing under $500k in Austin."},
    )
    assert send_response.status_code == status.HTTP_200_OK
    payload = send_response.json()
    assert "mandate agent" in payload["agent_message"]["content"].lower()
